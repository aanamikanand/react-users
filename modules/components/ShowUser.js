	...
	show() {
		...
		return(
			<div className="container row">
				<Link to="/" className="purple">Back</Link>
				<h2 className="center">{user.first_name}</h2>
				<p className="center"> Full Name: {user.first_name} {user.last_name}  </p>
				<p className="center">Phone No.: {user.phone_number}</p>
				<div className="center">
					<button className="btn purple" onClick={this.toggleEdit.bind(this)}>Edit</button>
				</div>
			</div>
		)
	}
